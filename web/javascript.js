 ...
 * @deprecated in favor of updateFile2SaveCsv
 */
...
/**
 * @author Saket Shukla <invincible.shukla@gmail.com>
 * @description Transpaoses an array of data
 * @param {Array<Array<any>>} matrix - 2D irregular array
 * @returns {Array<Array<any>>} which is the same 2D array with transposed rows and columns 
 */
function transposeMatrix(matrix) {
    const maxLen = matrix.reduce((max, {length}) => Math.max(max, length), 0);
    const output = Array.from({ length: maxLen }, (_, colIndex) => matrix.map(row => row[colIndex]));
    return output;
}

/**
 * @author Saket Shukla <invincible.shukla@gmail.com>
 * @description Converts a 2D array of data to a csv format string
 * @param {Array<Array<any>>} inputArray - 2D array of data with each column representing a set
 * @returns {string} of the CSV formatted content
 */
function arrayToCSV(inputArray) {
    let outputString = inputArray.map(inputArrayItem => inputArrayItem.join(',')).join('\r\n');
    return outputString;
}

/**
 * @author Saket Shukla <invincible.shukla@gmail.com>
 * @description Updtaes the string that represents the sets in form of a text to be exported as a csv format in an .ivenn file. The download is generated by the user and this is conidered on the interaction.
 */
function updateFile2SaveCsv() {
    let fileContent = "";
    let contentArray = [];
    // Create data set into an array with each set name and data to be in a single row
    for (let i = 0; i < nWay; i++) {
        contentArray.push([]);
        var setID = allPossibleSetsNames[i];
        var setname = d3.select("#label" + setID).text();
        contentArray[i].push(setname);
        size = original_sets[setID].length;
        for (var j = 0; j < size; j++) {
            contentArray[i].push(original_sets[setID][j]);
        }
    }
    // Transpose data so the set data is arranged in columns
    contentArray = transposeMatrix(contentArray);
    // Convert data array to csv string
    fileContent = arrayToCSV(contentArray);
    document.getElementById("data_set_content").value = fileContent;
}
